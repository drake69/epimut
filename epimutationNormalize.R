...
epimut_normalize_champ <- function(sampleFolder, resultFolder, methodNormalization, useMvalue = FALSE)
  {
  # if(endsWith(sampleFolder, suffix ="/"))
  # {
  #   message("Sample folder must not end with slash!")
  #   return()
  # }
  ...
  loadMethod <- if ( methodNormalization=="SWAN") {"minfi"} else {"ChAMP"}
  ...
  normalizedData <- champ.norm(
    ...
    resultsDir = paste(normalizationDataFolder, "/CHAMP_Normalization_by_", methodNormalization , "/", sep = ""),
    method = methodNormalization,
    plotBMIQ = FALSE,
    #arraytype = "450K",
  ...
  probeFeaturesValue <- probeFeaturesValue[
    order(probeFeaturesValue[, "Probe"], decreasing = FALSE),
  ]

  names(probeFeaturesValue)[names(probeFeaturesValue) == "MAPINFO"] <- "START"
  
  normalizedData <- normalizedData[
    order(row.names(normalizedData), decreasing = FALSE),
  ]
  ...
  #browser()
  sampleSheet <- if (methodNormalization=="SWAN") {data.frame(dataToNormalize$pd@listData)} else { dataToNormalize$pd }
...
epimut_normalize_rnbeads <- function(sampleFolder, resultFolder, methodNormalization)
{
  ...
  normalizedData2 <- normalizedData2[
    order(row.names(normalizedData2), decreasing = FALSE),
    ]
